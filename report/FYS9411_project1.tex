...
\end{equation}
where $\widetilde{\delta\vec{r}}$ is a stochastic vector drawn uniformly from the interval $-\delta r \leftrightarrow \delta r$ in each direction of space for some fixed step size $\delta r$. The proposal distribution $P$ in this case is symmetric,
\begin{equation}
...
\begin{equation}
\frac{\delta\vec{r}}{\delta t} = \frac{1}{2}\vec{Q}[\vec{r}]+\widetilde{\vec{\eta}}, \label{Langevin}
\end{equation}
where $\vec{Q}$ is a dimensionless drift term and $\widetilde{\vec{\eta}}$ is a dimensionless stochastic term drawn from a normal distribution with zero mean and unit deviation. Discretising this with the Euler-Maruyama method (a modified Euler method for stochastic differential equations) gives a new algorithm for proposing moves for the random particle $i$ through
\begin{equation}
\vec{r'}_i = \vec{r}_i+\frac{1}{2}\vec{Q}_i[\vec{R}]\delta t + \widetilde{\vec{\eta}}\sqrt{\delta t} \label{propLD}
\end{equation}
for some fixed small (dimensionless) time step $\delta t$. The proposal distribution $P$ in this case is not symmetric, but can be found by considering the corresponding Fokker-Planck equation, which is a partial differential equation describing the time evolution of the spatial configuration distribution $\Pi$ when the Langevin equation is invoked. The Fokker-Planck equation corresponding to the overdamped Langevin equation \eqref{Langevin} is also known as the Smoluchowski equation and is given by
\begin{equation}
...
\begin{equation}
G[\delta t, \vec{R}'_i|\vec{R}] = \sqrt{\frac{1}{2\pi \delta t}}^{DN} \epsilon^{-\frac{\left(\vec{r'}_i-\vec{r}_i-\frac{1}{2}\vec{Q}_i[\vec{R}]\delta t\right)^2}{2\delta t}} \label{Green}
\end{equation}
...
\begin{align}
&\left(\vec{r'}_i-\vec{r}_i-\frac{1}{2}\vec{Q}_i[\vec{R}]\delta t\right)^2-\left(\vec{r}_i-\vec{r'}_i-\frac{1}{2}\vec{Q}_i[\vec{R}'_i]\delta t\right)^2 \nonumber\\
&= -\left(\vec{r'}_i-\vec{r}_i\right) \cdot \frac{\vec{Q}_i[\vec{R}'_i]+\vec{Q}_i[\vec{R}]}{2}\delta t-\frac{\vec{Q}_i^2[\vec{R}'_i]-\vec{Q}_i^2[\vec{R}]}{4}\delta t^2 \nonumber\\
&= -\left(\vec{r'}_i-\vec{r}_i+\frac{\vec{Q}_i[\vec{R}'_i]-\vec{Q}_i[\vec{R}]}{2}\delta t\right)\cdot\frac{\vec{Q}_i[\vec{R}'_i]+\vec{Q}_i[\vec{R}]}{2}\delta t \nonumber
\end{align}
the acceptance probability in \eqref{acc_i} reduces in this case to
\begin{align}
&A[\vec{R}'_i|\vec{R}] = \min\Bigg\{1, \nonumber\\
&\prod\limits_{j \neq i} \frac{g^2[\vec{r'}_i]f^2[\Delta r'_{ij}]}{g^2[\vec{r}_i]f^2[\Delta r_{ij}]}\epsilon^{-\frac{\vec{Q}_i[\vec{R}'_i]+\vec{Q}_i[\vec{R}]}{4}\cdot\left(\vec{r'}_i-\vec{r}_i+\frac{\vec{Q}_i[\vec{R}'_i]-\vec{Q}_i[\vec{R}]}{2}\delta t\right)}\Bigg\} \label{accLD}
\end{align}
...
\end{align}
as long as all $\Delta r_{ij} > a$. This last restriction is why the hard-sphere interaction energy $V$ is omitted. For configurations in which $\Delta r_{ij} \leq a$ for some $i$ and $j$, the trial wavefunction $\Psi$ is defined to be zero (because $f[\Delta r_{ij}]$ is zero), which in turn means that the Metropolis algorithm will never sample such configurations, and so the fact that the local energy is infinite for such configurations (because $V$ is infinite) is not a problem.
